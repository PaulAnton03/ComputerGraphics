    ...
    */
    if (features.extra.enableMotionBlur && !features.extra.refitAABB) {
        ...
        if (denom.x != 0) {
            ...
            pos3.x = p1.x * 2.f * pos3.x * (1 - pos3.x) + p2.x * pos3.x * pos3.x;
        }
        if (denom.y != 0) {
            pos3.y = -p1.y / denom.y;
            if (pos3.y < 0) {
            ...
            pos3.y = p1.y * 2.f * pos3.y * (1 - pos3.y) + p2.y * pos3.y * pos3.y;
        }
        if (denom.z != 0) {
            pos3.z = -p1.z / denom.z;
            if (pos3.z < 0) {
            ...
            pos3.z = p1.z * 2.f * pos3.z * (1 - pos3.x) + p2.z * pos3.z * pos3.z;
        }
        aabb.lower += minOfThree(p2, glm::vec3 { 0 }, pos3);
...
void BVH::buildNumLevels()
{
    ...
    uint32_t curLevel = 0;
    while (!childQueue.empty() || !curQueue.empty()) {
        if (curQueue.empty()) {
            curQueue = childQueue;
...
void BVH::debugDrawLevel(int level)
{
    ...
    while (!curQueue.empty()) {
        drawAABB(m_nodes[curQueue.front()].aabb, DrawMode::Wireframe, glm::vec3(1.0f, 1.0f, 0.05f), 1.0f);
        curQueue.pop();
    }
}
...
void BVH::debugDrawLeaf(int leafIndex)
{
    ...
    if (leafIndex < 0 || index > m_numLeaves)
        return;
    int j = 0;
    uint32_t ind = RootIndex;
    while (ind < m_nodes.size() && j < leafIndex) {
    ...
    ind--;
    if (j != leafIndex || ind >= m_nodes.size() || !m_nodes[ind].isLeaf())
        return;
