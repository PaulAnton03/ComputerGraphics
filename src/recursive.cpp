...
Ray generateReflectionRay(Ray ray, HitInfo hitInfo)
{
    ...
    Ray res = { ray.origin + ray.direction * ray.t + newDir*0.00005f, newDir, std::numeric_limits<float>::max() };
    return res;
}
...
{
    Ray res = { ray.origin + ray.direction * ray.t + ray.direction * 0.00005f, ray.direction, std::numeric_limits<float>::max() };
