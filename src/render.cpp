...
std::vector<Ray> generatePixelRaysMultisampled(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int i = 0; i < numSamples; i++) {
        rays.push_back(camera.generateRay((glm::vec2(pixel) + state.sampler.next_2d()) / glm::vec2(screenResolution) * 2.f - 1.f));
    }
    return rays;
}
...
std::vector<Ray> generatePixelRaysStratified(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int p = 0; p < numSamples; p++) {
        for (int q = 0; q < numSamples; q++) {
            rays.push_back(camera.generateRay((glm::vec2(pixel) + (glm::vec2{p,q} + state.sampler.next_2d())/(float)numSamples) / glm::vec2(screenResolution) * 2.f - 1.f));
        }
    }
    return rays;
}